...
class SignedModuloInstructionSequence(InstructionSequence):
    ...
    def handle_match(
        ...
        if not mul:
            ...
            return match
        match = self.handle_magic_numbers_division(match, original_constants, original_registers, sequence)
        return match
    ...
    def handle_magic_numbers_division(
        ...
        match.constant = None
        match.constant = self._get_original_constant_from_magic(original_constants, original_registers, sequence[:len(match.sequence)])
        return match
    ...
    def _get_original_constant_from_magic(
        ...
        if not quotient:
            quotient = self.magic_table_long.get((magic, power + 64))
        return quotient
